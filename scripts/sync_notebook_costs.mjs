...
async function run() {
  ...
  const userItems = userList.trim().split('\n').map(line => {
    // Parse from right to left to get the price accurately correctly
    // Line example: 15939-5 NOTEBOOK ACER ASPIRE A14-52M-72FH ULTRA7-256V 16/1TB/14" / 695.00 |
    ...
    namePart = namePart.replace(/^\d+-\d+\s+/, '').replace('NOTEBOOK ', '').trim();

    return { cleanName: namePart, price };
  }).filter(Boolean);

  console.log('| Notebook (Usuario) | Precio Cargado (USD) |');
  console.log('| :--- | :--- |');

  let updated = 0;
  for (const u of userItems) {
    const keywords = u.cleanName.toLowerCase().replace(/[\/"]/g, ' ').split(' ').filter(w => w.length > 2);
    
    // Find best match in DB
    const match = dbProducts.find(p => {
       const dbName = p.name.toLowerCase();
       // Primary match: Product model code (e.g. A14-52M or F1404ZA)
       // Let's take keywords that look like specific model IDs
       const modelKeywords = keywords.filter(k => /[A-Z0-9-]{5,}/i.test(k));
       if (modelKeywords.length > 0 && modelKeywords.some(mk => dbName.includes(mk))) return true;
       
       // Fallback: Broad match with 3+ keywords
    ...
    if (match) {
      ...
      updated++;
      console.log('| ' + u.cleanName + ' | ' + u.price + ' |');
    } else {
      console.log('| ' + u.cleanName + ' | ⚠️ No encontrado |');
    }
  }

  console.log('\nSincronización completada: ' + updated + ' notebooks actualizadas.');
  process.exit(0);
}
