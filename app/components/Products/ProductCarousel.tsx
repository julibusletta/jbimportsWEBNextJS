...
export function ProductCarouselSection({ title, section, progressColor = '#0066cc' }: ProductCarouselSectionProps) {
  const [products, setProducts] = useState<any[]>([]); // Use any temporarily to handle mixed data formats
  ...
  return (
    <section id={sectionId} className="carrusel py-4 md:py-10 bg-gradient-to-br from-gray-100 to-white" data-aos="fadeInUp">
      <div className="max-w-[1400px] mx-auto px-4 w-full">
        ...
        {/* Carousel Wrapper inside the constraint */}
          ...
          {!loading && products.length > 0 && (
            <Slider {...settings}>
              {products.map((product) => {
                ...
                const displayImage = product.image || (product.imageUrls && product.imageUrls.length > 0 ? product.imageUrls[0] : '/images/placeholder.png');
                
                // Handle different discount formats
                const discountPct = product.discountPercentage || (product.discount ? Math.round((product.discount / (product.price + product.discount)) * 100) : 0);
